...
bool Manager::addRelationship(const string &ID_start, const string &ID_target, const string &relationship)
{
  ...
  if((relationship==relation::parent)&&(_checkAge(ID_target, ID_start)))    //Così la aggiunge, ma anche a me sembra strano
    return false;
  if((relationship==relation::born)&&(_checkAge(ID_start, ID_target)))
    return false;
